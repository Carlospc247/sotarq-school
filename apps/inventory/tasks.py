...
def get_monthly_depreciation_cost():
    """
    Calcula o custo total de depreciação de todos os ativos para o mês atual.
    Este valor deve ser subtraído do lucro bruto.
    """
    total_monthly_loss = Decimal('0.00')
    active_assets = Asset.objects.all()
    
    for asset in active_assets:
        # Depreciação Linear: (Preço - Resíduo) / (Anos * 12 meses)
        monthly = (asset.purchase_price - asset.residual_value) / (asset.category.depreciation_years * 12)
